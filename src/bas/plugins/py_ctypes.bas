...
_    'T0 _      '*< The first type block
  ...
  , IMPORTS _ '*< The list of Python imports
...
SUB py_CTOR CDECL(BYVAL O AS Options PTR)
  HEADER = "# header file auto generated by fbdoc and plugin py_ctypes.bas" _
...
SUB EmitterInit CDECL(BYVAL Emi AS EmitterIF PTR, BYREF Par AS STRING) EXPORT
  ...
  a = INSTR(Par, !"\t-pyimports=")
  IF a THEN
    a += 12
    VAR e = INSTR(a, Par, !"\t")
    IF 0 = e THEN e = LEN(Par) + 1
    IMPORTS = TRIM(MID(Par, a,  e - a), ANY "'""")
