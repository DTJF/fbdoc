...
SELECT CASE AS CONST .NamTok[3]
CASE .TOK_BROPN : MSG_END("START: " & .SubStr(.NamTok) & "()")
CASE .TOK_DOT   : MSG_END("START: " & .SubStr(.NamTok) & "." & .SubStr(.NamTok + 6) & "()")
END SELECT
    ...
    IF LEN(LOFN) THEN
      ...
      WHILE t < e
        VAR c = t[1] - 1
        IF *t >= .TOK_ABST ANDALSO .Buf[c] <> ASC("@") THEN
          VAR l = t[2] _
            , x = l _
            , s = @.Buf[c + 1]
            ', s = UCASE(MID(.Buf, c + 2, l))
...
s = @.Buf[c + 1]
            ...
            var a = t[1], e = a + l - 1
            'x = l - 1
            'WHILE s[x] = (LOFN[i] AND &b11011111)
            WHILE (.Buf[e] AND &b11011111) = (LOFN[i] AND &b11011111)
'if murx then MSG_END("  murx: " & (s[x] AND &b11011111) & "=" & (LOFN[i] AND &b11011111)) 
              'x -= 1
              e -= 1
              i -= 1
              'IF x < 0 THEN
              IF e < a THEN
                ...
                CASE ASC(!"\n")
MSG_END("  NL: " & MID(.Buf, a + 1, l))

                CASE ASC(  ".")

if .Buf[c]     = asc(">") andalso _
   .Buf[c - 1] = asc("-") then _
_    'x = *(t - 1) - 1 : s -= x + 3 : l += x + 2 : i -= 1 : _
    e -= 2 : a = e - *(t - 1) + 1 : l += e - a + 2 : i -= 1 : _
    MSG_END("  HIER: " & MID(.Buf, a + 1, l)) : _
_    MSG_END("  HIER: " & .SubStr(t-3) & "." & MID(.Buf, c + 2, l) & " -> " & mid(peek(zstring, s), 1, l) & "..." & mid(LOFN, i - x + 1, l)) : _
_    MSG_END("  HIER: " & .SubStr(t-3) & "." & MID(.Buf, c + 2, l) & " -> " & mid(peek(zstring, s), 1, l) & "..." & (s[x] AND &b11011111) & "=" & (LOFN[i] AND &b11011111) & " " & x & " " & i) : _
murx = 1 : _
    continue while

''if .Buf[c] = asc(".") orelse _
'if *(t - 3) = .TOK_DOT then _
'MSG_END("  HIER: " & .SubStr(t-6) & "." & MID(.Buf, c + 2, l) & " -> " & chr(.Buf[c]))

                  x = i
                  DO : i -= 1 : LOOP UNTIL LOFN[i] = ASC(!"\n") ORELSE i < 1
                  l += x - i
                CASE ELSE : EXIT WHILE
                END SELECT
                cEmitComments(P, c + 1)
                ...
                EXIT FOR
              END IF
            'WEND : DO : i -= 1 : LOOP UNTIL LOFN[i] = ASC(!"\n") ORELSE i < 1
            WEND : WHILE i > 0 andalso LOFN[i] <> ASC(!"\n") : i -= 1 : wend
          NEXT
        END IF : t += 3
      WEND
    END IF
