...
\Proj operates on input and generates output. Both can either be
...
this files, calling the related emitter functions. So \Proj uses at
...
running in mode \ref SubSecModDefault. It gets generated by \Proj in
...
When \Proj runs as a Doxygen filter (default mode), it gets called for
...
To generate those function calls in the intermediate format, \Proj
checks the words in a function body. If a word is a symbol (no keyword),
it can either be an array or a function call, \Proj needs further
...
\Proj operates on a single file only, the names of the functions in
...
\Proj emitter \ref SecEmmSyntax also reads informations from that
...
In run mode \ref SubSecModSyntax \Proj reads special html files
...
In run mode \ref SubSecModSyntax \Proj reads special tex files
generated by Doxygen, containing the source code listings. Those files
contain links to the description text for symbols in the source. This
information gets extract from the original files and copied to the
newly generated files with correct syntax highlighting. During this
...
In run mode \ref SubSecModSyntax \Proj reads special xml files
generated by Doxygen, containing the source code listings. Those files
contain links to the description text for symbols in the source. This
information gets extract from the original files and copied to the
newly generated files with correct syntax highlighting. During this
...
emitter \ref SecEmmCSource. \Proj searches for that file in current
...
Such files aren't necessary for basic usage of \Proj. They are used
for external emitters to extend the \Proj features and get loaded on
